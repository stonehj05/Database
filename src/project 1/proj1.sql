...
AS
  ...
  WHERE s.AB > 50
  ORDER BY s.slgval DESC, yearid, p.playerid
  LIMIT 10
;
...
-- Question 4iii
CREATE VIEW salarastatistics(yearid, min, max, avg)
AS
  SELECT yearid, MIN(salary), MAX(salary), avg(salary)
  FROM salaries
  GROUP BY yearid
...
AS
  ...
  FROM salarastatistics s1 INNER JOIN salarastatistics s2
  ON s1.yearid - 1 = s2.yearID
  ORDER BY s1.yearid
;

-- Question 4iv
CREATE VIEW playerdata(playerid, yearid, salary)
AS
...
AS
  SELECT p.playerid, p.namefirst, p.namelast, d.salary, d.yearid 
  FROM people p INNER JOIN playerdata d
  ON p.playerid = d.playerid
  GROUP BY d.yearid
  HAVING d.salary = MAX(d.salary) 
;
-- Question 4v
